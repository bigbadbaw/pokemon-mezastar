import { type PokemonType, type Effectiveness } from "./types";

const typeChart: Record<PokemonType, Record<PokemonType, Effectiveness>> = {
...
export function getTypeEffectiveness(
  ...
  return defenderTypes.reduce(
    (multiplier, defType) => multiplier * typeChart[attackType][defType],
    1,
  );
}
...
export function getSuperEffectiveTypes(
  ...
  return allTypes.filter(
    (atkType) => getTypeEffectiveness(atkType, defenderTypes) >= 2,
  );
}
...
export function getResistantTypes(
  ...
  const allTypes = Object.keys(typeChart) as PokemonType[];
  return allTypes.filter(
    (atkType) => getTypeEffectiveness(atkType, defenderTypes) < 1,
  );
}
